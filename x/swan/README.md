...
This document specifies the swan module of the Cosmos SDK.

The swan module is responsible for handling multi-asset coin transfers between
...
In addition, the swan module tracks and provides query support for the total
...
and the swan `Keeper` that are related to `ModuleAccount`s in order to be able
to:
...
The `x/swan` module keeps state of the following primary objects:
...
In addition, the `x/swan` module keeps the following indexes to manage the
...
The swan module stores it's params in state with the prefix of `0x05`,
it can be updated with governance or the address with authority.

* Params: `0x05 | ProtocolBuffer(Params)`

```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/swan/v1beta1/swan.proto#L12-L23
```

## Keepers

The swan module provides these exported keeper interfaces that can be
...
Best practices dictate careful review of `swan` module code to ensure that
permissions are limited in the way that you expect.

### Denied Addresses

The `x/swan` module accepts a map of addresses that are considered blocklisted
...
By providing the `x/swan` module with a blocklisted set of addresses, an error occurs for the operation if a user or client attempts to directly or indirectly send funds to a blocklisted account, for example, by using [IBC](https://ibc.cosmos.network).
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/swan/v1beta1/tx.proto#L38-L53
```
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/swan/v1beta1/tx.proto#L58-L69
```
...
The `swan` module params can be updated through `MsgUpdateParams`, which can be done using governance proposal. The signer will always be the `gov` module account address. 

```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/swan/v1beta1/tx.proto#L74-L88
```
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/swan/v1beta1/tx.proto#L96-L117
```
...
The swan module emits the following events:
...
In addition to message events, the swan keeper will produce events when the following methods are called (or any method which ends up calling them)
...
The swan module contains the following parameters
...
A user can query and interact with the `swan` module using the CLI.

#### Query

The `query` commands allow users to query `swan` state.

```shell
simd query swan --help
```
...
```shell
simd query swan balances [address] [flags]
```

Example:

```shell
simd query swan balances cosmos1..
```
...
```shell
simd query swan denom-metadata [flags]
```

Example:

```shell
simd query swan denom-metadata --denom stake
```
...
```shell
simd query swan total [flags]
```

Example:

```shell
simd query swan total --denom stake
```
...
```shell
simd query swan send-enabled [denom1 ...] [flags]
```

Example:

```shell
simd query swan send-enabled
```
...
The `tx` commands allow users to interact with the `swan` module.
...
```shell
simd tx swan send [from_key_or_address] [to_address] [amount] [flags]
```

Example:

```shell
simd tx swan send cosmos1.. cosmos1.. 100stake
```

## gRPC

A user can query the `swan` module using gRPC endpoints.
...
```shell
cosmos.swan.v1beta1.Query/Balance
```
...
```shell
cosmos.swan.v1beta1.Query/AllBalances
```
...
```shell
cosmos.swan.v1beta1.Query/DenomMetadata
```
...
```shell
cosmos.swan.v1beta1.Query/DenomsMetadata
```
...
```shell
cosmos.swan.v1beta1.Query/DenomOwners
```
...
```shell
cosmos.swan.v1beta1.Query/TotalSupply
```
...
```shell
cosmos.swan.v1beta1.Query/SupplyOf
```
...
The `Params` endpoint allows users to query the parameters of the `swan` module.

```shell
cosmos.swan.v1beta1.Query/Params
```
...
The `SendEnabled` enpoints allows users to query the SendEnabled entries of the `swan` module.

Any denominations NOT returned, use the `Params.DefaultSendEnabled` value.

```shell
cosmos.swan.v1beta1.Query/SendEnabled
```
